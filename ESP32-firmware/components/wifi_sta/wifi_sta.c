...
static void try_next_network(void)
{
    ...
    if (s_network_index == 0 && WIFI_LIST_SIZE > 1) {
        ESP_LOGW(TAG, "All networks failed — backing off 5s");
        vTaskDelay(pdMS_TO_TICKS(5000));
    }

    wifi_config_t cfg = {0};
    strlcpy((char *)cfg.sta.ssid, wifi_list[s_network_index].ssid,
            sizeof(cfg.sta.ssid));
    strlcpy((char *)cfg.sta.password, wifi_list[s_network_index].password,
...
void wifi_init(void)
{
    ...
    strlcpy((char *)wifi_cfg.sta.ssid, wifi_list[0].ssid,
            sizeof(wifi_cfg.sta.ssid));
    strlcpy((char *)wifi_cfg.sta.password, wifi_list[0].password,
