...
{
    if (ads == NULL || bus_handle == NULL || *bus_handle == NULL)
    {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    {
        i2c_master_bus_rm_device(ads->i2c_handle);
        ads->i2c_handle = NULL;
    }

    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = addr & 0x7F,
        .scl_speed_hz = i2c_frequenzy,
    };

    ads->address = dev_cfg.device_address;
    ...
    esp_err_t ret = i2c_master_bus_add_device(*bus_handle, &dev_cfg, &ads->i2c_handle);
    if (ret != ESP_OK)
    {
        return ret;
    }

    ret = write_register(ads, ADS_REG_CONFIG_ADDRESS, ads->config);
    if (ret != ESP_OK)
    {
        i2c_master_bus_rm_device(ads->i2c_handle);
        ads->i2c_handle = NULL;
...
static uint16_t measure_differential(ads1115_t *ads, uint16_t mux_setting)
{
    ...
    for (int i = 0; i < 50; ++i)
    {
        if (read_register(ads, ADS_REG_CONFIG_ADDRESS, &status) != ESP_OK)
        {
            return 0;
        }

        if ((status & ADS_REG_CONFIG_OS_MASK) != 0)
        {
            break;
        }
    ...
    if (read_register(ads, ADS_REG_CONVERSION_ADDRESS, &raw_value) != ESP_OK)
    {
        return 0;
    }
...
uint16_t ads1115_get_raw(ads1115_t *ads, uint8_t channel)
{
    ...
    {
    case 0:
        mux_setting = ADS_REG_CONFIG_MUX_0_GND;
        break;
    case 1:
        mux_setting = ADS_REG_CONFIG_MUX_1_GND;
        break;
    case 2:
        mux_setting = ADS_REG_CONFIG_MUX_2_GND;
        break;
    case 3:
        mux_setting = ADS_REG_CONFIG_MUX_3_GND;
        break;
    default:
        return 0;
    }
