  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSub}>Usage & Billing Overview</Text>
      </View>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summaryRow}>
          ...
          <View style={styles.summaryCard}>
            ...
            <View style={styles.valueRow}>
              <Text style={[styles.summaryValue, { color: '#0f766e' }]}>EGP {weekTotalCost.toFixed(2)}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.summaryRow}>
          <View style={styles.summaryCard}>
            ...
            <View style={styles.valueRow}>
              <Text style={styles.summaryValue}>{last6MonthsKwh.reduce((a, b) => a + b, 0)}</Text>
              <Text style={styles.summaryUnit}>kWh</Text>
            </View>
          </View>
          <View style={styles.summaryCard}>
            <Text style={styles.summaryLabel}>Total Cost</Text>
            <View style={styles.valueRow}>
              <Text style={[styles.summaryValue, { color: '#0f766e' }]}>EGP {last6MonthsCost.reduce((a, b) => a + b, 0).toFixed(2)}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.tableCard}>
          ...
          <View style={styles.tableHeader}>
            ...
            <Text style={[styles.tableHeaderText, { flex: 1, textAlign: 'center' }]}>kWh</Text>
            <Text style={[styles.tableHeaderText, { flex: 1, textAlign: 'center' }]}>Cost</Text>

          </View>
          {last6MonthsLabels.map((month, index) => {
            const rate = last6MonthsKwh[index] > 0 ? (last6MonthsCost[index] / last6MonthsKwh[index]).toFixed(2) : '0.00';
            return (
              <View key={month} style={[styles.tableRow, index === last6MonthsLabels.length - 1 && styles.tableRowLast]}>
                <Text style={[styles.tableCell, { flex: 1 }]}>{month}</Text>
                <Text style={[styles.tableCell, { flex: 1, textAlign: 'center' }]}>{last6MonthsKwh[index]}</Text>
                <Text style={[styles.tableCell, { flex: 1, textAlign: 'center', color: '#0f766e' }]}>EGP {last6MonthsCost[index].toFixed(2)}</Text>
...
const styles = StyleSheet.create({
  ...
  header: { padding: 20, paddingTop: 16, borderBottomWidth: 1, borderBottomColor: '#e2e8f0' },
  ...
  summaryUnit: { fontSize: 14, color: '#64748b', fontWeight: '600', marginLeft: 4 },
  ...
  chartBadge: { backgroundColor: '#ecfdf5', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999 },
  ...
  tableHeader: { flexDirection: 'row', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#e2e8f0' },
  tableHeaderText: { fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.8, fontWeight: '700' },
